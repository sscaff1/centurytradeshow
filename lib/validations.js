  ...
  _.each(security.workTimes, function(workTime) {
    ...
    if (workTime !== {}) {
      if (!workTime.type || isNaN(workTime.personnel) || !workTime.booth || !workTime.startTime || !workTime.endTime) {
        ...
        errors.workTime = 'One of your start times is after or at the same time as your end time.';
...
validateCareerForm = function(careers) {
  var errors = {};
  _.each(careers, function(career) {
    if (career.required === 'required' && !career.value) {
      errors[career.name] = 'You  must fill out this field or select a value.';
    }
  });
  return errors;
}
